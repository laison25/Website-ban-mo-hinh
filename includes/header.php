...
<header class="site-header">
    <div class="container nav-wrap">
        ...
        <div class="nav-actions">
            <form action="<?= url('index.php') ?>" method="get" class="search-form">
                <input type="text" name="keyword" placeholder="What are you looking for?" value="<?= e($_GET['keyword'] ?? '') ?>">
                <button type="submit" aria-label="Tìm kiếm">🔎</button>
            </form>
            <a class="icon-link" href="<?= url('cart.php') ?>">🛒 <span class="counter"><?= cart_count() ?></span></a>
            <?php if ($user): ?>
                <div class="user-chip">
                    <div>
...
<?php if ($flash): ?>
    <div class="container"><div class="flash-message <?= e($flash['type']) ?>"><?= e($flash['message']) ?></div></div>
<?php endif; ?>
