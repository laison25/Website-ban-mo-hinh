...
if ($keyword !== '') {
    $filtered = array_filter($products, function ($product) use ($keyword) {
        return stripos($product['name'], $keyword) !== false
            ...
            || stripos($product['studio'], $keyword) !== false;
    });
}
...
<main>
  ...
  <section class="section-space" id="flash-sale">
    <div class="container">
      ...
      <div class="section-head">
        <div>
          <h2>Flash Sales</h2>
          <div class="countdown">
            <div><span data-unit="d">03</span><small>Days</small></div>
            <div><span data-unit="h">23</span><small>Hours</small></div>
            <div><span data-unit="m">19</span><small>Minutes</small></div>
            <div><span data-unit="s">56</span><small>Seconds</small></div>
          </div>
        </div>
        <a href="#best-selling" class="pill-btn">View All</a>
      </div>

      <?php if (empty($flashSale)): ?>
        <div class="empty-box">Khong tim thay san pham phu hop voi tu khoa "<?php echo htmlspecialchars($keyword); ?>".</div>
      <?php else: ?>
        <div class="product-grid">
          <?php foreach ($flashSale as $product): ?>
            <article class="product-card">
              <a href="product-detail.php?id=<?php echo $product['id']; ?>" class="product-image">
                <img src="<?php echo $product['image']; ?>" alt="<?php echo htmlspecialchars($product['name']); ?>">
                <span class="badge-sale">-15%</span>
              </a>
              <div class="product-meta">
                <div class="studio"><?php echo htmlspecialchars($product['studio']); ?></div>
                <h3><a href="product-detail.php?id=<?php echo $product['id']; ?>"><?php echo htmlspecialchars($product['name']); ?></a></h3>
                <div class="price-line">
                  <strong><?php echo number_format($product['price'], 0, ',', '.'); ?>d</strong>
                  <span><?php echo number_format($product['old_price'], 0, ',', '.'); ?>d</span>
                </div>
                <form class="card-cart-form" method="post" action="cart.php">
                  <input type="hidden" name="action" value="add">
                  <input type="hidden" name="product_id" value="<?php echo (int) $product['id']; ?>">
                  <input type="hidden" name="quantity" value="1">
                  <button type="submit" class="outline-btn card-cart-btn">Add to cart</button>
                </form>
              </div>
            </article>
          <?php endforeach; ?>
  ...
  <section class="section-space" id="best-selling">
    <div class="container">
      <div class="section-label">This Month</div>
      <div class="section-head">
        <h2>Best Selling Products</h2>
        <a href="index.php" class="pill-btn">View All</a>
      </div>
      <div class="product-grid">
        <?php foreach ($bestSelling as $product): ?>
          <article class="product-card">
            <a href="product-detail.php?id=<?php echo $product['id']; ?>" class="product-image">
              <img src="<?php echo $product['image']; ?>" alt="<?php echo htmlspecialchars($product['name']); ?>">
              <span class="badge-ribbon">Best Seller</span>
            </a>
            <div class="product-meta">
              <div class="studio"><?php echo htmlspecialchars($product['studio']); ?></div>
              <h3><a href="product-detail.php?id=<?php echo $product['id']; ?>"><?php echo htmlspecialchars($product['name']); ?></a></h3>
      ...
      <div class="center-box"><a href="#" class="outline-btn">Xem them san pham pokemon</a></div>
    </div>
  </section>

  <section class="service-row section-space-sm" id="about">
    <div class="container service-grid">
      <div class="service-item"><div class="service-icon">🚚</div><h4>FREE AND FAST DELIVERY</h4><p>Free delivery for all orders over 5M</p></div>
